...
if ($_GET['aksi'] == 'tambah') {
    ...
    if(!in_array($filetype, array_keys($allowedTypes))) {
        ...
        if (move_uploaded_file($tmp_name, $target_path)) {
            ...
            $sql = "INSERT INTO `mobil`(`j_game`, `j_ps`, `harga`, `deskripsi`, `status`, `gambar`) 
                VALUES (?,?,?,?,?,?)";
...
if ($_GET['aksi'] == 'edit') {
    ...
    $data[] = $id;
    $sql = "UPDATE mobil SET j_game= ?, j_ps=?, harga=?, deskripsi=?, status=?, gambar=?
        WHERE id_ps = ?";
    $row = $koneksi->prepare($sql);
    $row->execute($data);
...
if (!empty($_GET['aksi'] == 'hapus')) {
    ...
    $sql = "DELETE FROM mobil WHERE id_ps = ?";
    $row = $koneksi->prepare($sql);
