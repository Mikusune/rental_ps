...
if ($_GET['aksi'] == 'tambah') {
    ...
    if(!in_array($filetype, array_keys($allowedTypes))) {
        ...
        if (move_uploaded_file($tmp_name, $target_path)) {
            ...
            $sql = "INSERT INTO `playstation`(`j_game`, `j_ps`, `harga`, `deskripsi`, `status`, `gambar`) 
                VALUES (?,?,?,?,?,?)";
            $row = $koneksi->prepare($sql);
            $row->execute($data);
            echo '<script>alert("sukses");window.location="PS.php"</script>';
        } else {
...
if ($_GET['aksi'] == 'edit') {
    ...
    $data[] = $id;
    $sql = "UPDATE playstation SET j_game= ?, j_ps=?, harga=?, deskripsi=?, status=?, gambar=?
        WHERE id_ps = ?";
    $row = $koneksi->prepare($sql);
    $row->execute($data);
...
if (!empty($_GET['aksi'] == 'hapus')) {
    ...
    $sql = "DELETE FROM playstation WHERE id_ps = ?";
    $row = $koneksi->prepare($sql);
    $row->execute(array($id));

    echo '<script>alert("sukses hapus");window.location="PS.php"</script>';
}
